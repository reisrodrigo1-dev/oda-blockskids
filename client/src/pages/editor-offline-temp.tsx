  ...
  const compileAndUpload = async () => {
    ...
    try {
      ...
      const uploadToUno = async (hexData: string) => {
        ...
        // Comando 'p' - Enter programming mode
        console.log('📡 Enviando comando AVR109: Enter programming mode');
        await sendData(new Uint8Array([0x70])); // 'p'
        await new Promise(resolve => setTimeout(resolve, 100));

        // Comando 'a' - Auto-increment
        console.log('📡 Enviando comando AVR109: Auto-increment');
        await sendData(new Uint8Array([0x61])); // 'a'
        await new Promise(resolve => setTimeout(resolve, 100));

        // Enviar dados do programa
        const programData = parseIntelHex(hexData);
        console.log('💾 Enviando programa - Total bytes:', programData.length);

        // Para Optiboot, enviar dados byte por byte
